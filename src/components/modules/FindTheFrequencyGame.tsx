...
interface FindTheFrequencyGameProps {
  submoduleId: string
...
/**
 * FindTheFrequencyGame - Play the exact octave on piano
 * ⭐⭐ Tier 2 Game for Module 1.2
 */
export const FindTheFrequencyGame: React.FC<FindTheFrequencyGameProps> = ({
  submoduleId,
  notes,
  questionCount = 5,
  timerSeconds,
  ...
  const [feedbackType, setFeedbackType] = useState<
    'correct' | 'wrong-note' | 'wrong-octave' | null
  >(null)
  ...
  const expectedNoteLetter = currentNote?.replace(/\d/, '')
  const expectedOctave = currentNote?.match(/\d/)?.[0]

  // Unique note letters in this level
  const allowedNoteLetters = useMemo(() => {
    const letters = notes.map((n) => n.replace(/\d/, ''))
  ...
  // Render the current note on staff (optional hint)
  useEffect(() => {
  ...
  const handlePianoPress = useCallback(
    (playedNote: string) => {
      ...
      setSelectedAnswer(playedNote)

      const playedLetter = playedNote.replace(/\d/, '')
      const playedOctave = playedNote.match(/\d/)?.[0]
      ...
      } else if (playedLetter === expectedNoteLetter && playedOctave !== expectedOctave) {
        // Right note, wrong octave (partial credit possible)
        setStreak(0)
        setFeedbackType('wrong-octave')
      } else {
        // Wrong note entirely
        setStreak(0)
        setFeedbackType('wrong-note')
      }

      setShowResult(true)
      setCountdown(AUTO_ADVANCE_DELAY)
    },
    [showResult, currentNote, expectedNoteLetter, expectedOctave]
  )
    ...
    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-[#30e8e8]">Find the Frequency</span>
        </div>
      ...
      <div className="text-center mb-6">
        <h3 className="text-lg font-semibold text-slate-800 dark:text-white mb-1">Hãy chơi nốt</h3>
        <p className="text-4xl font-bold text-[#30e8e8] mb-1 animate-pulse">{currentNote}</p>
        <p className="text-xs text-slate-400">Play this exact note on the piano</p>
      </div>

      {/* Optional staff hint (smaller) */}
      <div className="flex justify-center mb-4">
        <div
          ref={staffRef}
          className="bg-white dark:bg-slate-900 rounded-lg p-2 border border-slate-200 dark:border-slate-700 min-h-[60px] opacity-50"
        />
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
        <VirtualPiano
          ...
          onStopNote={() => {}}
          activeNotes={selectedAnswer ? [selectedAnswer] : []}
          allowedNotes={allowedNoteLetters}
        />
        <p className="text-xs text-center text-slate-400 mt-2">
          💡 C notes are marked with dots • Only {allowedNoteLetters.join(', ')} keys active
        </p>
      </div>
        ...
        <div
          className={`p-4 rounded-xl text-center font-bold mt-4 ${
            feedbackType === 'correct'
              ...
              : feedbackType === 'wrong-octave'
                ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300'
                : 'bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-300'
          }`}
        >
          {feedbackType === 'correct' ? (
            <>✓ Perfect! You played {currentNote}</>
          ) : feedbackType === 'wrong-octave' ? (
            <>
              ⚠️ Close! Right note, wrong octave. You played {selectedAnswer}, needed {currentNote}
            </>
          ) : selectedAnswer ? (
            <>
              ✗ Wrong! You played {selectedAnswer}, the answer was {currentNote}
            </>
          ) : (
...
export default FindTheFrequencyGame
