...
const FingeringChart: React.FC<FingeringChartProps> = ({
  ...
  const renderHole = (state: HoleState, index: number, totalHoles: number) => {
      ...
      return (
        <g key={index}>
          {/* Hole Label */}
          <text x="15" y={cy} dy="5" className="text-xs fill-stone-500 font-mono text-center font-bold" textAnchor="middle">{holeNumber}</text>
          {holeGraphic}
        </g>
      );
  };
  ...
  return (
    <div className={`relative ${className}`} style={{ width: '120px', margin: '0 auto' }}>
      <svg viewBox={`0 0 100 ${totalHeight}`} className="w-full h-auto drop-shadow-xl overflow-visible">
        <defs>
        ...
        {/* Flute Body */}
        <rect x="30" y="10" width="40" height={totalHeight - 20} rx="6" fill="url(#fluteGradient)" className="stroke-stone-400 stroke-1" />

        {/* Bamboo Joints (Decorative lines) */}
        <line x1="30" y1="55" x2="70" y2="55" className="stroke-stone-400/50 stroke-1" />
        <line x1="30" y1={totalHeight/2} x2="70" y2={totalHeight/2} className="stroke-stone-400/50 stroke-1" />

        {/* Blow Hole (Decorative) */}
        <ellipse cx="50" cy="35" rx="10" ry="8" className="fill-stone-800" />
        <text x="85" y="40" className="text-[9px] fill-stone-400 font-serif italic">Thổi</text>
